...
def _normalize_scopes(raw: str) -> List[str]:
    ...
    for line in raw.splitlines():
        ...
        normalized = SCOPE_LABEL_MAP.get(key) or SCOPE_LABEL_MAP.get(value.replace("`", "").strip())
        if normalized and normalized not in scopes:
            scopes.append(normalized)
    return scopes


def _build_entry(file_path: Path, source_title: str, section_heading: str, body: str) -> SkillLibraryEntry | None:
    name = _extract_fenced_block(body, ["名称", "建议名称"]).splitlines()
    name_value = name[0].strip() if name else ""
    if not name_value:
        cleaned_heading = re.sub(r"^Skill\s*\d+\s*[：:]\s*", "", section_heading, flags=re.IGNORECASE)
        name_value = cleaned_heading.strip()

    instructions = _extract_fenced_block(body, ["规则文本", "instructions", "可直接放入 instructions"])
    if not instructions:
        return None
...
async def get_skill_library() -> SkillLibraryResponse:
    ...
    for file_path in sorted(SKILL_LIBRARY_DIR.glob("*.md")):
        try:
            ...
            runtime.LOGGER.warning("Failed to load skill library file %s: %s", file_path.name, exc)
